...
void RTI::begin() {
  ...
  for (int i = 0; i < RTI_NEIGHBOUR_COUNT; i++) {
    byte nDID = DEVICE_OFFSET + (2 * i + 1);  // NEIGHBOUR ID start at 1 on ODD side
...
void receive(message_t* incoming) {
  ...
  if (incoming->nNID == NET_PREFIX &&
    ...
    info.sPending = true;
  } else if (incoming->nNID == NET_PREFIX &&
    incoming->nDID == info.fID){
    if (info.sSkip){
      if (millis()- rstStamp > SKIP_TIMEOUT) {
        info.sPending;
        info.sSkip = false;
      }
    } else {
